...
my $f3 = "3.query_speciesID_taxID.txt";#all genomes species ID and taxon ID extacted from NCBI taxdump using taxonkit
open IN3, $f3 or die;
    ...
    if ($type == 1)
      {
        ...
        if (-e $accession_file1) {print "$s\_$type\_Accession_no_species.txt exists\n";$FLAG = 0;} else {open OUTPUT5, ">$s\_$type\_Accession_no_species.txt" or die;	$FLAG = 1;}
        if (-e $accession_file2) {print "$g\_$type\_Accession_no_species.txt exists\n";$FLAG = 0;} else {open OUTPUT6, ">$g\_$type\_Accession_no_species.txt" or die;	$FLAG = 1;}
        if (-e $accession_file3) {print "$f\_$type\_Accession_no_species.txt exists\n";$FLAG = 0;}	else {open OUTPUT7, ">$f\_$type\_Accession_no_species.txt" or die;	$FLAG = 1;}
      }
    ...
    if ($type == 2)
      {
        ...
        my $accession_file2 = "$g\_$type\_Accession_no_species.txt";
        my $accession_file3 = "$f\_$type\_Accession_no_species.txt";
        if (-e $accession_file1) {print "$s\_$type\_Accession_no_species.txt exists\n";$FLAG = 0;} else {open OUTPUT8, ">$s\_$type\_Accession_no_species.txt" or die;	$FLAG = 1;}
        if (-e $accession_file2) {print "$g\_$type\_Accession_no_species.txt exists\n";$FLAG = 0;} else {open OUTPUT9, ">$g\_$type\_Accession_no_species.txt" or die;	$FLAG = 1;}
        if (-e $accession_file3) {print "$f\_$type\_Accession_no_species.txt exists\n";$FLAG = 0;}	else {open OUTPUT10, ">$f\_$type\_Accession_no_species.txt" or die;	$FLAG = 1;}
      }
...
sub alien_genes_finder
  {
    ...
    open OUTPUT11, ">$f4\_phyletic_pattern_analysis_native_alien_genes.txt";
    ####Type1 data
    ...
    foreach my $a (sort keys %final_native_alien_genes)
        {
            ...
            # print OUTPUT11 "$a\t$species_level{$a}{\"phyletic_distribution_threshold\"}\t$genus_level{$a}{\"phyletic_distribution_threshold\"}\t$family_level{$a}{\"phyletic_distribution_threshold\"}\t$final_native_alien_genes{$a}{\"type\"}\n";
            # print OUTPUT11 "$a\t$species_level{$a}{\"phyletic_distribution_threshold\"}\t$genus_level{$a}{\"phyletic_distribution_threshold\"}\t$family_level{$a}{\"phyletic_distribution_threshold\"}\t$final_native_alien_genes{$a}{\"type\"}\n";
        }
...
usage:
  ...
  perl APP.pl -q <fileName> -o <Output fileName> -f <fileType> [Options]
...
-v - Provide detailed progress messages.
...
1. Input multifasta file
  perl APP.pl -q example/NC_00.faa -o NC_00 -f multifasta
2. Accession number of genomes
  perl APP.pl -q NC_00 -o NC_00 -f accession
BLOCK
}
